...
const Dashboard = ({ isSidebarOpen, setTodoList }) => {
  ...
  // Fetch data dan konversi ke format kalender
  useEffect(() => {
    const fetchSchedules = async () => {
      if (!token) {
        console.log("No token found, cannot fetch schedules.");
        return;
      }
      ...
      try {
        const response = await fetch("http://localhost:3000/api/v1/schedules", {
          ...
          headers: {
            ...
            Authorization: `Bearer ${token}`, // Bearer token untuk autentikasi
          },
        });
        ...
        // Format the fetched data for the calendar
        const formattedEvents = data.map((task) => ({
          title: task.taskName,
          start: new Date(task.executeAt),
          end: new Date(task.executeAt), // If it is a single day event
          allDay: false,
          id: task.id, // Store the schedule ID for later use
        }));
  ...
  const handleDateClick = async (event) => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/v1/schedules/${event.id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        // Navigasi ke halaman detail schedule atau lakukan apa yang diperlukan dengan data schedule
  ...
  return (
    <div
      className={`transition-all duration-300 p-6 mt-20 grid gap-6 ${isMobile ? "grid-cols-1" : "grid-cols-3"
        } ${isSidebarOpen ? "ml-64" : "ml-20"}`}
    >
      {/* Filter Section */}
      <div className="col-span-4 flex justify-start items-center bg-white p-4 rounded-md shadow-md w-fit">
      ...
      <div className="col-span-2 bg-white p-6 rounded-md shadow-md h-[500px] w-full overflow-x-auto">
        ...
        <div className="flex justify-between items-center mb-4 space-x-2">
          ...
          <div className="flex space-x-2">
            <button
              onClick={() => setView(Views.MONTH)}
              className={`p-2 rounded-md text-sm ${view === Views.MONTH ? "bg-purple-500 text-white" : "bg-gray-100"
                }`}
            >
              Month
            </button>
            <button
              onClick={() => setView(Views.WEEK)}
              className={`p-2 rounded-md text-sm ${view === Views.WEEK ? "bg-purple-500 text-white" : "bg-gray-100"
                }`}
            >
              Week
            </button>
            <button
              onClick={() => setView(Views.DAY)}
              className={`p-2 rounded-md text-sm ${view === Views.DAY ? "bg-purple-500 text-white" : "bg-gray-100"
                }`}
            >
              Day
            </button>
        ...
        <Calendar
          ...
          onView={setView}
          toolbar={false}
          onSelectEvent={handleDateClick} // Handle the click event on a calendar date
          components={{
            header: ({ label }) => (
              <div className="bg-purple-500 text-white p-2 font-bold">
                {label}
      ...
      {/* Action Buttons */}
      <div className="col-span-1 bg-white p-6 rounded-md shadow-md flex justify-between items-center">
        <button
